...
func (ts *ValetTestSuite) TestLoadConfig_NoFile() {
	...
	ts.Equal(slog.LevelInfo, cfg.LogLevel.Level, "expected LogLevel=info by default")
}
...
func (ts *ValetTestSuite) TestLoadConfig_FromFile() {
	...
	ts.Equal(slog.LevelDebug, cfg.LogLevel.Level, "expected LogLevel=debug from config file")
}
...
`
	...
	// Verify values
	ts.Equal(slog.LevelDebug, cfg.LogLevel.Level, "LogLevel should be debug")
